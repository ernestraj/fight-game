 ...
 */
function ge_rubik_preprocess_html() {
 ...
 */
function ge_rubik_css_alter(&$css) {
  if (isset($css['modules/overlay/overlay-child.css'])) {
    $css['modules/overlay/overlay-child.css']['data'] = drupal_get_path('theme', 'ge_rubik') . '/overlay-child.css';
  }
  if (isset($css['modules/shortcut/shortcut.css'])) {
    $css['modules/shortcut/shortcut.css']['data'] = drupal_get_path('theme', 'ge_rubik') . '/shortcut.css';
  }
  ...
  if (isset($css['sites/all/modules/views/css/views-admin.rubik.css'])) {
    $css['sites/all/modules/views/css/views-admin.rubik.css']['data'] = drupal_get_path('theme', 'ge_rubik') . '/views-admin.rubik.css';
  }
}
...
function ge_rubik_theme() {
  ...
  $items['comment_wrapper'] = array(
    'path' => drupal_get_path('theme', 'ge_rubik') .'/templates',
    'template' => 'object',
  );
  ...
  $items['user_admin_access_add_form'] = array(
    ...
    'preprocess functions' => array(
      'ge_rubik_preprocess_form_buttons',
    ),
  );
  ...
  $items['confirm_form'] = array(
    ...
    'preprocess functions' => array(
      'ge_rubik_preprocess_form_confirm'
    ),
  );
 ...
 */
function ge_rubik_preprocess_page(&$vars) {
  //Replaced clone content link on top with rules links clone 
  if (!empty($vars['action_links'])) {
    if ($vars['action_links'][0]['#link']['title'] == t('Clone content')){
      $cl_nid = arg(1);
      unset($vars['action_links']);
      $vars['action_links'] = $vars['page']['content']['system_main']['nodes'][$cl_nid]['rules_links_clone'];
      unset($vars['page']['content']['system_main']['nodes'][$cl_nid]['rules_links_clone']);
    }
  }
  elseif(empty($vars['action_links'])){
    $cl_nid = arg(1);
    if (isset($vars['page']['content']['system_main']['nodes'][$cl_nid]['rules_links_clone'])) {
      $vars['action_links'] = $vars['page']['content']['system_main']['nodes'][$cl_nid]['rules_links_clone'];
      unset($vars['page']['content']['system_main']['nodes'][$cl_nid]['rules_links_clone']);
    }
  }
  ...
  $vars['page_icon_class'] = ($item = menu_get_item()) ? implode(' ' , _ge_rubik_icon_classes($item['href'])) : '';
 ...
 */
function ge_rubik_preprocess_fieldset(&$vars) {
 ...
 */
function ge_rubik_preprocess_form_buttons(&$vars) {
 ...
 */
function ge_rubik_preprocess_form_confirm(&$vars) {
 ...
 */
function ge_rubik_preprocess_form_node(&$vars) {
 ...
 */
function ge_rubik_preprocess_button(&$vars) {
 ...
 */
function ge_rubik_preprocess_help(&$vars) {
  $vars['hook'] = 'help';
 ...
 */
function ge_rubik_preprocess_help_page(&$vars) {
  $vars['hook'] = 'help-page';
 ...
 */
function ge_rubik_preprocess_node(&$vars) {
  $vars['layout'] = TRUE;
  $vars['submitted'] = _ge_rubik_submitted($vars['node']);
}

/**
 * Preprocessor for theme('comment').
 */
function ge_rubik_preprocess_comment(&$vars) {
  $vars['layout'] = TRUE;
  $vars['submitted'] = _ge_rubik_submitted($vars['comment']);
}

/**
 * Preprocessor for theme('comment_wrapper').
 */
function ge_rubik_preprocess_comment_wrapper(&$vars) {
  $vars['hook'] = 'box';
  $vars['layout'] = FALSE;
 ...
 */
function ge_rubik_preprocess_admin_block(&$vars) {
  // Add icon and classes to admin block titles.
  if (isset($vars['block']['href'])) {
    $vars['block']['localized_options']['attributes']['class'] =  _ge_rubik_icon_classes($vars['block']['href']);
  }
 ...
 */
function ge_rubik_breadcrumb($vars) {
  $output = '';
 ...
 */
function ge_rubik_filter_guidelines($variables) {
  return '';
}
 ...
 */
function ge_rubik_node_add_list($vars) {
  ...
  if ($content) {
    foreach ($content as $item) {
      ...
      if (isset($item['localized_options']['attributes']['class'])) {
        $item['localized_options']['attributes']['class'] += _ge_rubik_icon_classes($item['href']);
      }
      else {
        $item['localized_options']['attributes']['class'] = _ge_rubik_icon_classes($item['href']);
      }
 ...
 */
function ge_rubik_admin_block_content($vars) {
  $content = $vars['content'];
  ...
  if (!empty($content)) {
    ...
    foreach ($content as $k => $item) {
      ...
      if (!empty($content[$k]['localized_options']['attributes']['class'])) {
        $content[$k]['localized_options']['attributes']['class'] += _ge_rubik_icon_classes($item['href']);
      }
      else {
        $content[$k]['localized_options']['attributes']['class'] = _ge_rubik_icon_classes($item['href']);
      }
 ...
 */
function ge_rubik_admin_drilldown_menu_item_link($link) {
  ...
  if (!isset($link['localized_options']['attributes']['class'])) {
    $link['localized_options']['attributes']['class'] = _ge_rubik_icon_classes($link['href']);
  }
  else {
    $link['localized_options']['attributes']['class'] += _ge_rubik_icon_classes($link['href']);
  }
 ...
 */
function ge_rubik_preprocess_textfield(&$vars) {
 ...
 */
function ge_rubik_menu_local_task($variables) {
 ...
 */
function ge_rubik_render_clone($elements) {
  static $instance;
  if (!isset($instance)) {
    $instance = 1;
  }
 ...
 */
function _ge_rubik_submitted($node) {
 ...
 */
function _ge_rubik_icon_classes($path) {
  $classes = array();
  $args = explode('/', $path);
...
function _ge_rubik_local_tasks(&$vars) {
