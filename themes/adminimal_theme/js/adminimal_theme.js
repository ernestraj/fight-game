...
Drupal.behaviors.adminimal_media_queries = {
	attach: function (context, settings) {
		...
		// Hide the message for news article
		setTimeout(function(){ $('#news-article-node-form .messages').fadeOut('slow'); }, 5000);	
	}
};
...
Drupal.behaviors.adminimal_move_active_primary_tab = {
  ...
  Drupal.behaviors.adminimal_theme = { 
    attach: function(context,settings) {
      $(".disable-self-service-role .form-item.form-type-select").prepend("<div class = 'disable-transparent'></div>");
    }
  };
